...
async function testDatabaseConnection() {
  log.section(
    "1. Testing Database Connection - اختبار الاتصال بقاعدة البيانات"
  );
...
async function testConsultationBookings() {
  ...
  try {
    ...
    // Count bookings
    const [countResult] = await connection.query(
      "SELECT COUNT(*) as count FROM consultationBookings"
    );
...
async function testTemplates() {
  ...
  try {
    ...
    // Count templates
    const [countResult] = await connection.query(
      "SELECT COUNT(*) as count FROM templates WHERE isActive = 1"
    );
...
function generateSummary(results) {
  ...
  if (failedTests === 0) {
    ...
    console.log(
      `${colors.green}✅ All tests passed! Database is properly connected and working.${colors.reset}`
    );
    console.log(
      `${colors.green}✅ جميع الاختبارات نجحت! قاعدة البيانات متصلة وتعمل بشكل صحيح.${colors.reset}`
    );
    console.log(
      `${colors.green}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}\n`
    );
    ...
    console.log(
      `${colors.red}❌ Some tests failed. Please check the database connection.${colors.reset}`
    );
    console.log(
      `${colors.red}❌ بعض الاختبارات فشلت. يرجى التحقق من اتصال قاعدة البيانات.${colors.reset}`
    );
    console.log(
      `${colors.red}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}\n`
    );
