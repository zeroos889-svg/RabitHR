...
export default function ConsultationDetail() {
  ...
  } = trpc.consulting.getTicket.useQuery({ ticketId }, { enabled: ticketId > 0 });
  ...
  } = trpc.consulting.getTicketResponses.useQuery({ ticketId }, { enabled: ticketId > 0, refetchInterval: 5000 });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await addResponseMutation.mutateAsync({
        ...
        attachments: uploadedUrls.length > 0 ? JSON.stringify(uploadedUrls) : undefined,
      });
          ...
          <Card className="border-red-200 bg-red-50">
            <CardContent className="pt-6">
              <div className="text-center text-red-600">
                ...
                <p className="text-lg font-semibold">لم يتم العثور على الاستشارة</p>
                <p className="text-sm mt-2">الاستشارة المطلوبة غير موجودة أو تم حذفها</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4 mb-6 max-h-[500px] overflow-y-auto">
                  {responses.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">سيتم الرد على استشارتك في أقرب وقت</p>
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className={`text-xs ${isConsultant ? "text-gray-500" : "text-purple-100"}`}>
                                        ...
                                        <div key={idx}>
                                          {isImage ? (
                                            ...
                                            >
                                              ...
                                              <span className="text-sm">مستند مرفق</span>
                                            </a>
                                          )}
                                        </div>
                                      );
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <label htmlFor="file-upload">
                          <Button
                            ...
                            disabled={uploadingFiles || addResponseMutation.isPending}
                            asChild
                          >
                      ...
                      <Button
                        ...
                        disabled={addResponseMutation.isPending || uploadingFiles || (!message.trim() && selectedFiles.length === 0)}
                        className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700"
                      >
                        {(addResponseMutation.isPending || uploadingFiles) ? (
                          <>
                            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white ml-2" />
                            {uploadingFiles ? "جاري الرفع..." : "جاري الإرسال..."}
                          </>
                        ) : (
                          <>
            ...
            {canRate && (
              <Card className="border-yellow-200 bg-yellow-50">
                <CardHeader>
                  ...
                  <CardDescription>ساعدنا في تحسين خدماتنا بتقييمك</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    {rating > 0 && <span className="text-sm text-gray-600 mr-2">({rating}/5)</span>}
                  </div>
                  <Textarea
                  ...
                  >
                    {rateTicketMutation.isPending ? "جاري الإرسال..." : "إرسال التقييم"}
                  </Button>
                </CardContent>
              </Card>
            )}
              ...
              <Card className="border-green-200 bg-green-50">
                <CardContent className="pt-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-green-900">تم تقييم الاستشارة</p>
                      <div className="flex items-center gap-1 mt-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            className={`h-4 w-4 ${
                              star <= ticket.rating! ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                            }`}
                          />
                        ))}
                      ...
                      {ticket.feedback && (
                        <p className="text-sm text-gray-700 mt-2">{ticket.feedback}</p>
                      )}
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                {ticket.consultantId && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-700">المستشار المعين</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-700">عدد الردود</p>
                    <p className="text-sm text-gray-600">{responses.length} رد</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardHeader>
                <CardTitle className="text-lg text-blue-900">هل تحتاج مساعدة؟</CardTitle>
              </CardHeader>
              <CardContent>
