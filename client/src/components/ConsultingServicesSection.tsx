      ...
      <div className="container">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            استشارات متخصصة في الموارد البشرية من خبراء معتمدين - أكثر من{" "}
            {types?.consultationTypes?.length || 10} نوع استشارة
          </p>
        </div>
              ...
              >
                {isPopular && (
                  <div className="absolute top-4 left-4">
                    <span
                      className={`${colors.text.replace("text-", "bg-")} text-white text-xs px-2 py-1 rounded-full`}
                    >
                      الأكثر طلباً
                    </span>
                  </div>
                ...
                <div
                  className={`h-14 w-14 rounded-lg ${colors.bg} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                >
        ...
        <div className="text-center">
          <Link href="/consulting">
            <Button size="lg" className="gradient-primary text-white">
              ...
              {types?.consultationTypes?.length || 10})
              <ArrowRight className="mr-2 h-4 w-4" />
            </Button>
          </Link>
        </div>
